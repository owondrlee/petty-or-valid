...
// ─── System prompt — tone, logic, and output format from petty-or-valid-brain.md
...
app.post("/api/verdict", async (req, res) => {
  ...
  try {
    ...
    if (err instanceof SyntaxError) {
      res.status(500).json({ error: "The judges are arguing. Try again." });
    } else {
...
app.listen(PORT, () => {
  console.log(`petty or valid → http://localhost:${PORT}`);
});
